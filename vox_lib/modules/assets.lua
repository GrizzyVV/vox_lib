...
-- ── VEHICLE DEFINITION catalog (the DA_* HVehicleDefinition layer — distinct from the spawn-BP `vehicle` family) ───────
-- Demonstrates the generalized "`DA_*` native-definition layer" pattern (Matt 2026-07-15): enumerate a DataAsset CLASS via
-- the AssetRegistry, then `UObject.Load` each and read its config FIELDS (not just id+path). `HVehicleDefinition` (`DA_*`
-- under /HelixVehicles/Blueprints/Cars) is the authoritative vehicle CATALOG (8: Bent/Bugat/Cama/Chev_ZR/DefaultCar/
-- EmptyCar/H1/Mercedec_500E) carrying `VehicleType` (enum: CAR…), cameras, input, interaction, LOD attachments. ⚠ It is NOT
-- the spawn actor — only some defs have a spawn BP under /HelixVehicleAssets (the `vehicle` family, HVehicle-spawnable). So
-- the def catalog (mapping target) and the spawn pool (what HVehicle can spawn) are separate; reconcile per vehicle.
local VEHICLE_DEF_ROOT = "/HelixVehicles/Blueprints/Cars"

-- Enumerate the HVehicleDefinition catalog → { {id, path, vehicleType, name}, ... } (sorted by id), CACHED. Loads each DA to
-- read VehicleType; a DA_ whose VehicleType doesn't resolve is skipped (isolates real vehicle defs from other DA_ under root).
function lib.enumerateVehicleDefinitions(opts)
    opts = opts or {}
    if _cache.vehicledef and not opts.refresh then return _cache.vehicledef end
    local list = {}
    local ok, err = pcall(function()
        local ar = UE.UAssetRegistryHelpers.GetAssetRegistry()
        local _, assets = ar:GetAssetsByPath(opts.root or VEHICLE_DEF_ROOT, true, true)
        if not (assets and assets.Length) then return end
        for i = 1, assets:Length() do
            local ad = assets:Get(i)
            local nm = tostring(ad.AssetName)
            if nm:sub(1, 3) == "DA_" then
                local path; pcall(function() path = tostring(ad.PackageName) .. "." .. nm end)
                local vtype
                if path then
                    local def; pcall(function() def = UE.UObject.Load(path) end)
                    if def then pcall(function() vtype = tostring(def.VehicleType) end) end
                end
                if vtype then
                    list[#list + 1] = { id = nm, path = path, vehicleType = vtype, name = nm:gsub("^DA_", "") }
                end
            end
        end
    end)
    if not ok then return nil, "vehicle-definition enumeration failed: " .. tostring(err) end
    table.sort(list, function(a, b) return a.id < b.id end)
    _cache.vehicledef = list
    return list
end

-- Write the vehicle-definition catalog to a curatable `.lua`. Returns ok,err. Server/admin only.
function lib.writeVehicleDefTable(filepath, opts)
    if type(filepath) ~= "string" then return false, "filepath required" end
    local list, err = lib.enumerateVehicleDefinitions(opts)
    if not list then return false, err end
    local lines = { "-- Auto-generated by vox_lib lib.writeVehicleDefTable() — the LIVE HVehicleDefinition catalog.", "return {" }
    for _, v in ipairs(list) do
        lines[#lines + 1] = string.format("  { id = '%s', name = '%s', vehicleType = '%s', path = '%s' },",
            v.id, v.name, tostring(v.vehicleType), v.path or "")
    end
    lines[#lines + 1] = "}"
    local f, ioerr = io.open(filepath, "w")
    if not f then return false, "cannot open file: " .. tostring(ioerr) end
